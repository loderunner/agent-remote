...
/**
 * Configuration for Kubernetes remote
 */
export type RemoteConfig = {
  /**
   * Name of the Kubernetes pod to execute commands on
   */
  pod: string;
  /**
  ...
  namespace?: string;
  /**
   * Container name within the pod (optional, uses default container if not specified)
   */
  container?: string;
  /**
...
export type ToolConfig = Required<Omit<RemoteConfig, 'container'>> & {
  container?: string;
};

/**
 * Kubernetes remote manager that provides tools for executing commands and
 * managing files on Kubernetes pods.
 *
 * @example
 * ```typescript
...
export class Remote {
   ...
   *
   * @param name - Optional name for the MCP server (defaults to 'remote-k8s')
   ...
   * ```
   */
  createSdkMcpServer(name: string = 'remote-k8s') {
